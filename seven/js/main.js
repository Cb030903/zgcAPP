	...
	function iss(data) {
		...
		if(retflag != undefined && retflag != null) {
			...
			if(str != null) {
				...
				!$('.in-header-bod h2 span').is('.active') ? a.text('* * * * *') && b.text('* * * ') && c.text('* * * ') && $('.in-header-bod h2 span').removeClass('active') : a.text(str.BODY.balance) && b.text(str.BODY.past_proceed) && c.text(str.BODY.total_proceed) && $('.in-header-bod h2 span').addClass('active');
				$('.in-cen-left p').text(str.BODY.proceed_rate + '%')
				$('.in-hb-tite p').text('零钱：' + str.BODY.sof_bal + '元')
				$('.in-header-box p').text(str.BODY.home_word)
			}
		}
		if(!$('.indexmi').eq(1).is(":hidden")) {
