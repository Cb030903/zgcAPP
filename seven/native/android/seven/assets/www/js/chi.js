...
//	setTimeout(function() {
		...
		var awi = awin / 2;
		var bwin = awi * 50;
		var sval = sessionStorage.getItem('sval') ? sessionStorage.getItem('sval') : '0.00';
		if(sval !== '' && !$('.in-bot-a').is('.active')) {
		...
		$('.number').on('input porpertychange', function() {
			...
			end !== '' ? $('.in-bot-num').addClass('action') && $('.in-bot-box').show().find('span').text(miv) : $('.in-bot-num').removeClass('action') && $('.in-bot-box').hide();
			end !== '' && !$('.in-bot-a').is('.active') ? $('.in-bot-b').css('background', '#4F85F3') : $('.in-bot-b').css('background', ' #c7d6f6');
